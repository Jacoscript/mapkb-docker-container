...
VIRTUOSO_T="/usr/local/virtuoso-opensource/bin/virtuoso-t -f"
ISQL="/usr/local/virtuoso-opensource/bin/isql localhost dba dba"
...
#        $CURL -X POST -H "Content-Type: application/json" -d '{ "dataStore": { "name": "NHDPlus", "description": "NHDPlus datastore", "connectionParameters": { "entry": [ {"@key":"url","$":"file:///makb_assets/data/nhdpl/Shape"} ] } } }' $REST/workspaces/usgsns/datastores
...
#        pkill java
        ...
        # Setup database - this is no longer necessary
        ...
        if [ ! -e /.is_development ]; then
            rm -rf /var/www/html/* 
            git clone https://code.usgs.gov/makb/MapKB.git /var/www/html/
            if [ ! -z ${BRANCH+x} ]; then 
                cd /var/www/html/
                git checkout $BRANCH 
            fi
